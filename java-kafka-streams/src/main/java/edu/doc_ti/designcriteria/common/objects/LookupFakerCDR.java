...
public class LookupFakerCDR {
    ...
    public static void loadFile(String filePath) {
        ...
        try (InputStream inputStream = LookupFakerCDR.class.getResourceAsStream(filePath);
               ...
               while ((line = br.readLine()) != null) {
            	   contador++ ;
                   ArrayList<String> values = new ArrayList<String> (Arrays.asList(line.replace(' ','_').split(";")) ) ;
            	   if (contador == 1) {
            		   headers.add(values);
            	   } else {
            		   myData.add(values) ;
...
//    	test() ;
    ...
    public static List<String> getAllHeaders() {
    	...
    	ArrayList<String> headersReturn = new ArrayList<String>() ;
    	...
    	for ( int posH = 0 ; posH< headers.size() ; posH++) {
    		...
    		for ( int posD = 0 ; posD< minSize ; posD++) {
    			headersReturn.add (myHeader.get(posD)) ;
        	}
    	}
    	
//    	System.out.println( headersReturn.size() ) ;
